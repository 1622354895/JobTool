...
<p align="center">
  ...
  <img alt="Tests" src="https://img.shields.io/badge/tests-31%20passed-2ea44f">
...
- **快速录入**：记录公司、岗位、状态、方向、渠道、地点、优先级和招聘链接。
- **批量解析**：支持键值文本、常见自然语言表达和一行一条的批量岗位信息。
- **写入前预览**：编辑或移除待写入记录，校验必填项并提示同批次重复数据。
- **投递管理**：按关键词、状态和岗位方向检索，更新进度并打开招聘链接。
- **跟进提醒**：记录沟通、笔试、面试和下一步行动，集中展示今日与逾期待办。
- **可视化看板**：统计累计投递、面试转化、状态、岗位方向、渠道和优先级分布。
- **Excel 自动化**：内置筛选、下拉选项、条件格式、指标卡、待办清单和四张图表。
...
| 跟进记录 | 沟通、笔试、面试和复盘记录 |
| 选项配置 | 状态、岗位方向、渠道等下拉选项 |

请勿修改工作表名称、主表表头或公式列，否则程序会拒绝读取不兼容的工作簿。
